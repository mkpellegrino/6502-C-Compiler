...
| tINC '(' ID ',' NUMBER ')'
{
  ...
  int l = atoi($5.name);
  for( int i = 0; i < l; i++ ) addAsm( str_INC + getNameOf( a ), size_of_instruction, false );
};
| tINC '(' expression ')'
{
...
| expression tbwNOT expression
{
  ...
  if( isWordID($1.name) && isWordID($3.name) )
    {
      addAsm( str_LDA + "#$FF", 2, false );
      addAsm( str_EOR + getNameOf(hexToDecimal($3.name)), 3, false );
      addAsm( str_AND + getNameOf(hexToDecimal($1.name)), 3, false );
      addAsm( str_PHA );
      addAsm( str_LDA + "#$FF", 2, false );
      addAsm( str_EOR + getNameOf(hexToDecimal($3.name)) + "+1", 3, false );
      addAsm( str_AND + getNameOf(hexToDecimal($1.name)) + "+1", 3, false );
      addAsm( str_TAX );
      addAsm( str_PLA );
      strcpy( $$.name, "_XA" );
    }
...
| tPEEK '(' expression ')'
{
  ...
  if( isARG($3.name) || isFAC($3.name) )
  ...
  else if( isWordID($3.name) )
    {
      ...
      addAsm( str_STA + "!+", 2, false );
      //addAsm( str_STA + "$02", 2, false );
      addAsm( str_STX + "!++", 2, false );
      //addAsm( str_STX + "$03", 2, false );
      //addAsm( str_LDA + "($02),Y", 2, false );
      addAsm( str_BYTE + "$AD", 3, false );
      addAsm( "!:", 0, true );
      addAsm( str_BYTE + "$00", 3, false );
      addAsm( "!:", 0, true );
      addAsm( str_BYTE + "$00", 3, false );
