  ...
  void Optimize()
  {
	...
	if( cmpstr( asm_instr[i]->getString(), cmt ) &&
	    ...
	    cmpstr( asm_instr[i+8]->getString(), cmt ) )
	  {
	    ...
	    addOptimizationMessage( "removing lack of ?? math FAC artifact (2)", i);
	  }
      }
...
| ID init 
{
  ...
  if(current_variable_base_address == -1)
    {
      addCompilerMessage( string("ERROR: Undeclared Variable: ") + $1.name, 3 );
    }
  addParserComment( "RULE: statement: ID init" );

  if( isWordID($1.name) && isA($2.name) )
    {
      addComment("WordID = A" );
  ...
  else if( isWordID($1.name) && isFAC($2.name) )
    {
      addComment( "FAC -> WORD" );
      addAsm( str_JSR + "$B1AA", 3, false );
      addComment("WordID = FAC" );
      addAsm( str_STY + getNameOf(getAddressOf($1.name)), 3, false );
      addAsm( str_STA + getNameOf(getAddressOf($1.name)) + " +1", 3, false );
    }  
    ...
    {
      addComment( "UintID init with A" );
      int instr_size = 3;
      if( current_variable_base_address < 256 ) instr_size=2; 
      addAsm( str_STA + getNameOf(current_variable_base_address) + commentmarker + "<<== here", instr_size, false);
    }
  ...
  else if( (isUintID($1.name)||isIntID($1.name)) && isFAC($2.name) )
    {
      addComment( "FAC -> WORD" );
      addAsm( str_JSR + "$B1AA", 3, false );
...

{
  ...
  if( isA($1.name) && isA($4.name) )
	...
	{
	  addCompilerMessage( "A math FAC: operation nyi", 3 );
	}
      strcpy( $$.name, "_FAC" );
    }
	...
	{	  
	  ...
	  addAsm( str_LDY + "#>!fp0+", 2, false );  
	  addAsm( str_JSR + "$B850" + commentmarker + "MEM - FAC -> FAC", 3, false );
	  addAsm( str_JSR + "$BFB4" + commentmarker + "-1 * FAC -> FAC", 3, false );
	  strcpy($$.name, "_FAC" );
	}
      else if( op == string("/") )
	{	  
	...
	{
	  addCompilerMessage("Math operation not implemented yet. (A arith FloatID)", 3);
	}
	...
	{
	  addCompilerMessage( "This is SO not worth is...", 0 );
	  addCompilerMessage( "Exponents NYI for A ** IntID", 3 );
	}
    }
  else if( isA($1.name) && isIntIMM($4.name) )
    {
      addCompilerMessage( "A math IntIMM: TOC", 0 );
	...
	{
	  addCompilerMessage( "This is SO not worth is...", 0 );
	  addCompilerMessage( "Exponents NYI for A ** IntIMM", 3 );
	}
    }
	...
	{
	  addCompilerMessage("Math operation not implemented yet. (A arith UintID)", 3);
	}
    }
      ...
      else if( op == string( "-" ) )
	{
	  ...
	  addAsm( str_LDY + "#>!fp0+", 2, false );
	  addAsm( str_JSR + "$BA8C" + commentmarker + "MEM -> ARG+", 3, false );
	  addAsm( str_JSR + "$B853" + commentmarker + "ARG - FAC -> FAC" );
	}
      else if( op == string( "*" ) )
	{
	  float_swap_space_is_needed = true;
      ...
      else if( op == string( "/" ) )
	{
	  ...
	  addAsm( str_LDY + "#>!fp0+", 2, false );
	  addAsm( str_JSR + "$BA8C" + commentmarker + "MEM -> ARG (+)", 3, false );	  
	  addAsm( str_JSR + "$BB12" + commentmarker + "ARG / FAC -> FAC" );
	}
      else if( op == string( "**" ) )
	{
	  float_swap_space_is_needed = true;
	  ...
	  addAsm( str_LDY + "#>!fp0+", 2, false );
	  addAsm( str_JSR + "$BA8C" + commentmarker + "MEM -> ARG+", 3, false );
	  addAsm( str_JSR + "$BF7B" + commentmarker + "ARG ** FAC -> FAC" );
	}
      else
	{
  ...
  else if( isFloatID($1.name) && isA($4.name) )
    {
      ...
      addCompilerMessage( "FloatID math A: TOC", 1 );
      ...
      else if( op == string("-"))
	{
	  ...
	  addAsm( str_LDA + "#<" + OP1, 2, false );
	  addAsm( str_LDY + "#>" + OP1, 2, false );
	  addAsm( str_JSR + "$B850" + commentmarker + "MEM - FAC -> MEM", 3, false );
	}
      else if( op == string("/"))
	{
	  addAsm( str_LDA + "#<" + OP2, 2, false );
	  ...
	  addAsm( str_JSR + "$BB0F" + commentmarker + "MEM / FAC -> MEM", 3, false );
	}
      else if( op == string( "**" ))
	{
	  addAsm( str_LDA + "#<" + OP1, 2, false );
    ...
    {
      addCompilerMessage( "FloatID math IntID: TOC", 1 );
      addAsm( str_LDX + "#$00", 2, false );
      ...
      if( op == string("*"))
	{
	  ...
	  addAsm( str_JSR + "$BA28" + commentmarker + "RAM * FAC -> FAC", 3, false );
	}
      else if( op == string("+"))
	{
	  inlineFloat($1.name);
	  addAsm( str_LDA + "#<" + getNameOf(getAddressOf($4.name)), 2, false );
	  addAsm( str_LDY + "#>" + getNameOf(getAddressOf($4.name)), 2, false );
	  addAsm( str_JSR + "$B867" + commentmarker + "RAM + FAC -> FAC", 3, false );
	}
      else if( op == string("-"))
	{
	  float_swap_space_is_needed = true;
	...
	{
	  addCompilerMessage("Math operation not implemented yet. (FloatIMM arith FloatIMM)", 3);
	}
	...
	{
	  addCompilerMessage("Math operation not implemented yet. (FloatIMM arith IntID)", 3);
	}
	...
	{
	  deletePreviousAsm();
	  addCompilerMessage( "deleting 'sec'", 1 );
	  addAsm( "// sec",0,false );
	}
      else if( op == "+" )
	{
	  deletePreviousAsm();
	  addCompilerMessage( "deleting 'clc'", 1 );
	  addAsm( "// clc",0,false );
	}
	...
	{
	  addCompilerMessage("Math operation not implemented yet. (FloatIMM arith WordID)", 3);
	}
      strcpy( $$.name, "_FAC" );
    }
	...
	{
	  addCompilerMessage("Math operation not implemented yet. (FloatIMM arith XA)", 3);
	}
      strcpy( $$.name, "_FAC" );
    }
    ...
    {      
      addComment( "IntID math FAC (in progress): TOC" );
      if( op == string( "+" ))
	{
	...
	{
	  addCompilerMessage( "IntID ** IntID --> XA not implemented", 3 );
	}
      else
	{
	...
	{
	  addCompilerMessage( "Arithmetic: Signed Int - Word may produce unexpected results", 0);
	  addComment( "IntID - WordID --> XA" );
	  addAsm( str_SEC, 1, false );
	  addAsm( str_LDA + "#$00", 2, false );
	...
	{
	  addComment( "A * UintID --> A" );
	  addCompilerMessage( "Arithmetic: Signed Int * Word NYI", 3 );
	  strcpy($$.name, "_XA");
	}
      else if( op == string("/") )
	{
    ...
    {
      addComment( "IntIMM math A (in progress): TOC" );
      int OP1value = twos_complement(atoi( stripFirst($1.name).c_str() ));
      if( op == string("+") )
	{
	  if( OP1value > 127 )
	    {
	      addAsm( str_ADC + "#$" + toHex(twos_complement(atoi(stripFirst($1.name).c_str()))), 2, false ); 
	      addAsm( str_TAY, 1, false );
	      addAsm( str_LDA + "#$FF", 2, false );
	      addAsm( str_ADC + "#$00", 2, false );
	      addAsm( str_TAX, 1, false );
	...
	{
	  addAsm( str_STA + "!+", 3, false );
	  addAsm( str_LDA + "#$" + toHex(twos_complement(atoi(stripFirst($1.name).c_str()))), 2, false );
      ...
      else if( op == string( "-" ) )
	{
	  ...
	  addAsm( str_LDY + "#$" + toHex(twos_complement(OP1value)), 2, false );
	  addAsm( str_LDA + "#$FF", 2, false );
	  addAsm( str_JSR + "$B391" + commentmarker + "WORD -> FAC", 3, false );
	  addAsm( str_LDX + "#<!fp1+", 2, false );
	  addAsm( str_LDY + "#>!fp1+", 2, false );
      ...
      else if( op == "+" )
	{
      ...
      addComment( "IntIMM math IntIMM: TOC" );
	...
	{
	  addCompilerMessage(string( "I am unable to recognize [") + op + string("]" ), 3 );
	}
    }

  else if( isIntIMM($1.name) && isUintIMM($4.name) )
    {
      addComment( "IntIMM math UintIMM: TOC" );
      int OP1 = atoi( stripFirst($1.name).c_str() );
      if( OP1 > 127 )
	{
	  OP1 = -twos_complement( OP1 );	  
    ...
    {      
      addComment( "UintID math FAC (in progress): TOC" );
      if( op == string( "+" ))
	{
	}
      else if( op == string( "-" ))
    ...
    {
      addComment( "UintID math IntID (nyi - fully): TOC" );
	...
	{
	  addCompilerMessage( "UintID / IntID is inefficient", 1 );
       	  addComment( "UintID / IntID --> XA" );
	  div16_is_needed = true;

	  addAsm( str_LDA + "#$38" + commentmarker + "<-- SEC Opcode", 2, false );	  
	  addAsm( str_STA + "!hack+", 3, false );
	  addAsm( str_LDA + O2, sizeOP2A, false );
	  addAsm( str_TAX, 1 , false );
	  addAsm( str_ASL, 1, false );
	  addAsm( str_BCC + "!skip+", 2, false );
	  addAsm( str_LDA + "#$18" + commentmarker + "<-- CLC Opcode", 2, false );	  
	  addAsm( str_STA + "!hack+", 3, false );
	  addAsm( str_LDA + O2, sizeOP2A, false );
	  addAsm( str_EOR + "#$FF", 2, false );
	  addAsm( str_CLC, 1, false );
	  ...
	  addAsm( "!hack:\t" + str_BYTE + "$00" + commentmarker + "<-- SEC or CLC depending on Sign from above ... this sounds positivly biblical!", 1, true );
	  addAsm( str_BCS + "!skip+", 2, false );
	  addAsm( str_LDA + "_DIV16_FB", 3, false );
	  addAsm( str_EOR + "#$FF", 2, false );
	  addAsm( str_CLC, 1, false );
	  addAsm( str_ADC + "#$01", 2, false );
	  addAsm( str_STA + "_DIV16_FB", 3, false );
	  addAsm( str_LDA + "_DIV16_FB +1", 3, false );
	  addAsm( str_EOR + "#$FF", 2, false );
	  addAsm( str_ADC + "#$00", 2, false );
	  addAsm( str_STA + "_DIV16_FB +1", 3, false );
	  addAsm( "!skip:\t", 0, true );
	  addAsm( str_LDA + "_DIV16_FB", 3, false );
	  addAsm( str_LDX + "_DIV16_FB +1", 3, false );
      ...
      else if ( op == string("*") )
	{
	  ...
	  addAsm( str_LDA + "MUL16R +1", 3, false );
	  addAsm( str_EOR + "#$FF", 2, false );
	  addAsm( str_ADC + "#$00", 2, false );
	...
	{
	  addCompilerMessage( "Unknown Math Operation: UintID math UintID", 3 );
	}
    }
	...
	{
	  //addCompilerMessage( "UintID ** UintIMM (nyi)", 3 );
	  int addr_op1 = hexToDecimal($1.name);
	  int op2 = atoi(stripFirst($4.name).c_str());
  ...
  else if( isUintID($1.name) && isWordIMM($4.name) )
    {
      ...
      addComment( "UintID math WordIMM (nyi - fully): TOC" );      
      int OP2 = atoi(stripFirst($4.name).c_str());
      if( op == string("+") )
	{
    ...
    {      
      addComment( "UintIMM math FAC (in progress): TOC" );
      if( op == string( "+" ))
	{
	}
      else if( op == string( "-" ))
  ...
  else if( isUintIMM($1.name) && isUintID($4.name) )     // IMM vs. ID
    {
	...
	{
	  addCompilerMessage( "UNSIGNED integer division (UintIMM / UintID) not yet implemented", 3 );
	}
      else
	{
    ...
    {      
      addComment( "WordID math FAC (in progress): TOC" );
      if( op == string( "+" ))
	{
	}
      else if( op == string( "-" ))
    ...
    {
      addComment( "WordID math IntID && WordID math UintID: TOC" );
      if( op == string("*") )
	{
    ...
    {
      addComment( "WordID math IntIMM && WordID math UintIMM: TOC" );
    ...
    {
      addComment( "WordIMM math UintIMM: TOC" );
      ...
      if( op == "-" )
	{
      ...
      addComment( "WordIMM math WordIMM: TOC" );
	...
	{
	  addCompilerMessage( "WordIMM ** XA uses only LB of Power", 1 );
	  mul16_is_needed = true;
	  pow16_is_needed = true;
	  addAsm( str_TAY, 1, false );	  
	  addAsm( str_LDA + "#$" + toHex(OP1L), 2, false );
    ...
    {      
      addComment( "XA math FAC (in progress): TOC" );
      if( op == string( "+" ))
	{
	}
      else if( op == string( "-" ))
  ...
  /* if they're BOTH Int or Uint or Word IMMs */
  else if( (isIntIMM($1.name) && isIntIMM($4.name)) ||
	   (isUintIMM($1.name) && isUintIMM($4.name)) ||
	   (isWordIMM($1.name) && isWordIMM($4.name)) )
    {
      if( op == "-" )
	{
	  deletePreviousAsm();
      ...
      addComment( "IntIMM math IntIMM or UintIMM math UintIMM or WordIMM math WordIMM: TOC" );
...
| tCOS '(' expression ')'
{
  if( isFloatIMM($3.name) )
    {
      inlineFloat($3.name);
      addCompilerMessage( "This should be hard-coded for efficiency.", 1 );
  ...
  if( byte2hex_is_needed )
    {
      ...
      addComment( "Undocumented Opcode (replaces the two lines above)" );
