...
string replaceAll(string str, const string &from, const string &to)
{
  ...
  void addCompilerMessage(string msg, int level = 0)
  {
    ...
    switch( level )
      {
      case 0:
	cerr << "\e[2m*** message *** line: " << countn+1 << " *** " << msg << " ***\e[22m" << endl;
	break;
      case 1:
	cerr
	  << "*** warning *** line: "
	...
	break;
      case 2:
	cerr << "\e[1m*** ERROR *** line: " << countn+1 << " *** " << msg << " ***\e[22m" << endl;
	break;
      case 3:
	cerr << "\e[1m***  C R I T I C A L   E R R O R  *** line: " << countn+1 << " *** " << msg << " ***\e[22m" << endl;
	exit(-1);
	break;
      }
    return;
...
condition: expression[LHS]
{
  ...
  if(isXA($LHS.name))
    {pushXA();}
  else if(isA($LHS.name))
    {pushA();}
  else if(isFAC($LHS.name))
    {pushFAC();}
} relop[OP] expression[RHS]
{
  if(  !isXA($RHS.name) &&
       !isA($RHS.name) &&
       !isFAC($RHS.name)
       &&
       (isA($LHS.name) || isXA($LHS.name) || isFAC($LHS.name))
       )
    {
      deletePreviousAsmUntil("// MARKED_FOR_DELETION");
      addCompilerMessage( "Deleted Mnemonics", 0 );
...
}
{
  ...
  addParserComment( scope_stack.top() + " " + $LHS.name + $OP.name + $RHS.name);
  addDebugComment( "=========================================================");
  string tc;
  if( isXA($LHS.name) ) tc+=string( "XA" );
  if( isA($LHS.name ) ) tc+=string( "A" );
  if( isUintID($LHS.name) ) tc+=string( "UintID" );
  if( isIntID($LHS.name) ) tc+=string( "IntID" );
  if( isWordID($LHS.name) ) tc+=string( "WordID" );
  if( isFAC($LHS.name) ) tc+=string( "FAC" );
  if( isFloatID($LHS.name) ) tc+=string( "FloatID" );
  if( isUintIMM($LHS.name) ) tc+=string( "UintIMM" );
  if( isIntIMM($LHS.name) ) tc+=string( "IntIMM" );
  if( isWordIMM($LHS.name) ) tc+=string( "WordIMM" );
  if( isFloatIMM($LHS.name) ) tc+=string( "FloatIMM" );
  tc+=string( " " );
  if( getAddressOf($LHS.name) == 0 )
    {
      tc+=$LHS.name;
    }
  else
    {
      tc+=string(getNameOf(getAddressOf($LHS.name)));
    }
  tc+=string( " relop " );
  if( isA($RHS.name ) )
    {
  ...
  if( isIntID($RHS.name) ) tc+=string( "IntID" );
  if( isWordID($RHS.name) ) tc+=string( "WordID" );
  if( isFAC($RHS.name) ) tc+=string( "FAC" );
  if( isFloatID($RHS.name) ) tc+=string( "FloatID" );
  if( isUintIMM($RHS.name) ) tc+=string( "UintIMM" );
  if( isIntIMM($RHS.name) ) tc+=string( "IntIMM" );
  if( isWordIMM($RHS.name) ) tc+=string( "WordIMM" );
  if( isFloatIMM($RHS.name) ) tc+=string( "FloatIMM" );
  tc+=string( " " );

  if( getAddressOf($RHS.name) <= 0 )
    {
      tc+=$RHS.name;
    }
  else
  ...
  if( isA($LHS.name) && isA($RHS.name) )
    ...
    {
      addComment( "A relop FAC: TOC" );

      if( string($OP.name) == string("<") )
	{
	  addCompilerMessage( "A relop FAC: Relative Operator Manipulation", 0 );
	  strcpy( $OP.name, ">" );
	}
  ...
  else if( isA($LHS.name) && isFloatID($RHS.name) )
    {
      addComment( "A relop FloatID: TOC" );
      string OP2 = getNameOf(getAddressOf($RHS.name));
      string OP2L = "#<" + OP2;
      string OP2H = "#>" + OP2;
    ...
    {
      addComment( "A relop FloatIMM: TOC" );
      if( string($OP.name) == string("<") )
	{
	  addCompilerMessage( "A relop FloatIMM: Relative Operator Manipulation", 0 );
	  strcpy( $OP.name, ">" );
	}
      else if( string($OP.name) == string(">") )
	{
  ...
  else if( isA($LHS.name) && isIntIMM($RHS.name))
    {
      ...
      if( string($OP.name) == "!=" )
	{
	  addCompilerMessage( "A is ALWAYS != to IntIMM", 1 );
	}
      if( string($OP.name) == ">" )
	{
	  addCompilerMessage( "A is ALWAYS > IntIMM", 1 );
	}
      if( string($OP.name) == ">=" )
	{
	  addCompilerMessage( "A is ALWAYS >= to IntIMM", 1 );
      ...
      if( string($OP.name) == "<" )
	{
	  addCompilerMessage( "A is NEVER < IntIMM", 1 );
	}
      if( string($OP.name) == "<=" )
	{
	  addCompilerMessage( "A is NEVER <= to IntIMM", 1 );
	}
      if( string($OP.name) == "==" )
	{
	  addCompilerMessage( "A is NEVER == to IntIMM", 1 );
  ...
  else if( isA($LHS.name) && isUintID($RHS.name))
    {
      addComment( "A relop UintID: TOC" );
      addAsm( str_CMP + getNameOf( getAddressOf($RHS.name) ), 3, false );
    }
  else if( isA($LHS.name) && isUintIMM($RHS.name))
    {
      addComment( "A relop UintIMM: TOC" );
  ...
  else if( isA($LHS.name) && isWordIMM($RHS.name) )
    {
      ...
      if( tmp_v > 255 )
	{
	  addCompilerMessage( "A is always less than this WordIMM", 1 );
	  addComment( "Set the Negative flag for '<'" );
  ...
  else if( isFAC($LHS.name) && isFloatID($RHS.name) )
    {
      ...
      if( string($OP.name) == string("<") )
	{
	  addCompilerMessage( "FAC relop FloatID: Relative Operator Manipulation", 0 );
	  strcpy( $OP.name, ">" );
	}
      else if( string($OP.name) == string(">") )
	{
  ...
  else if( isFAC($LHS.name) && isIntIMM($RHS.name) )
    {
      ...
      if( string($OP.name) == string("<") )
	{
	  addCompilerMessage( "FAC relop IntIMM: Relative Operator Manipulation", 0 );
	  strcpy( $OP.name, ">" );
	}
      else if( string($OP.name) == string(">") )
	{
  ...
  else if( isFAC($LHS.name) && isUintIMM($RHS.name) )
    {
      ...
      if( string($OP.name) == string("<") )
	{
	  addCompilerMessage( "FAC relop UintIMM: Relative Operator Manipulation", 0 );
	  strcpy( $OP.name, ">" );
	}
      else if( string($OP.name) == string(">") )
	{
  ...
  else if( isFAC($LHS.name) && isWordIMM($RHS.name) )
    {
      ...
      if( string($OP.name) == string("<") )
	{
	  addCompilerMessage( "FAC relop WordIMM: Relative Operator Manipulation", 0 );
	  strcpy( $OP.name, ">" );
	}
      else if( string($OP.name) == string(">") )
	{
  ...
  else if( isFloatID($LHS.name) && isA($RHS.name) )
    {
      addComment( "FloatID relop A: TOC" );
      addAsm( str_TAY, 1, false );
      addAsm( str_LDA + "#$00", 2, false );
      addAsm( str_JSR + "$B391" + commentmarker + "WORD -> FAC", 3, false );
  ...
  else if( isFloatID($LHS.name) && isFloatID($RHS.name) )
    {
    ...
    {
      addComment( "FloatID relop IntID: TOC" );
      addAsm( str_LDX + "#$00", 2, false );
      addAsm( str_LDA + getNameOf( getAddressOf( $RHS.name )), 3, false );
      addAsm( str_BPL + "!+", 2, false );
      addAsm( str_DEX, 1, false );
    ...
    {
      addComment( "FloatIMM relop FAC: TOC (testing)" );
      inlineFloat( $LHS.name );
      deletePreviousAsm();
      deletePreviousAsm();
      deletePreviousAsm();
    ...
    {      
      addComment( "FloatIMM relop FloatIMM: TOC" );
    ...
    {
      addComment( "FloatIMM relop IntIMM: TOC" );
      if( arg_show_opt ) addCompilerMessage( "This condition is known at compile-time... just hard-code it!", 0 );
      float L = atof( stripFirst($LHS.name).c_str() );
      float R = atof( stripFirst($RHS.name).c_str() );
      if( L > R )
  ...
  else if( isFloatIMM($LHS.name) && isUintID($RHS.name) )
    {
      addComment( "FloatIMM relop UintID: TOC" );
      inlineFloat( $LHS.name );
      deletePreviousAsm();
      deletePreviousAsm();
      deletePreviousAsm();
  ...
  else if( isFloatIMM($LHS.name) && isUintIMM($RHS.name) )
    {
      addComment( "FloatIMM relop UintIMM: TOC" );
      if( arg_show_opt ) addCompilerMessage( "This condition is known at compile-time... just hard-code it!", 0 );
      float L = atof( stripFirst($LHS.name).c_str() );
      float R = atof( stripFirst($RHS.name).c_str() );
      if( L > R )
  ...
  else if( isFloatIMM($LHS.name) && isWordID($RHS.name) )
    {
      addComment( "FloatIMM relop WordID: TOC" );
      inlineFloat( $LHS.name );
      deletePreviousAsm();
      deletePreviousAsm();
      deletePreviousAsm();
  ...
  else if( isFloatIMM($LHS.name) && isWordIMM($RHS.name) )
    {
      addComment( "FloatIMM relop WordIMM: TOC" );
      if( arg_show_opt ) addCompilerMessage( "This condition is known at compile-time... just hard-code it!", 0 );
      float L = atof( stripFirst($LHS.name).c_str() );
      float R = atof( stripFirst($RHS.name).c_str() );
      if( L > R )
  ...
  else if( isFloatIMM($LHS.name) && isXA($RHS.name) )
    {
      addComment( "FloatIMM relop XA: TOC" );
    ...
    {
      addComment( "IntID relop WordID: TOC (testing)" );
      addAsm( str_CLC, 1, false );
      addAsm( str_LDA + getNameOf(getAddressOf($RHS.name)) + " +1", 3, false );
      addAsm( str_BNE + "!+", 2, false );
      addAsm( str_LDA + getNameOf(getAddressOf($LHS.name)), 3, false );
    ...
    {
      int R = atoi( stripFirst($RHS.name).c_str() );
      addComment( "IntID relop WordIMM: TOC (testing)" );
      if( R > 127 )
	{
	  addCompilerMessage( "IntID relop WordIMM: This WordIMM will always be > IntID", 1 );
	  addAsm( str_LDA + "#$FF", 2, false );
	  addAsm( str_CLC, 1, false );
	}
      else
    ...
    {
      addComment( "IntIMM relop FAC: TOC" );
      inlineFloat( $LHS.name );
      deletePreviousAsm();
      deletePreviousAsm();
      deletePreviousAsm();
    ...
    {
      addComment( "IntIMM relop FloatIMM: TOC" );
      if( arg_show_opt ) addCompilerMessage( "This condition is known at compile-time... just hard-code it!", 0 );
      float L = atof( stripFirst($LHS.name).c_str() );
      float R = atof( stripFirst($RHS.name).c_str() );
      if( L > R )
    ...
    {
      addComment( "IntIMM relop IntIMM: TOC");
      if( arg_show_opt ) addCompilerMessage( "This condition is known at compile-time... just hard-code it!", 0 );
      float L = atof( stripFirst($LHS.name).c_str() );
      float R = atof( stripFirst($RHS.name).c_str() );
      if( L > R )
  ...
  else if( isIntIMM($LHS.name) && isUintID($RHS.name) )
    {
    ...
    {
      addComment( "IntIMM relop WordIMM: TOC" );
      if( arg_show_opt ) addCompilerMessage( "This condition is known at compile-time... just hard-code it!", 0 );
      float L = atof( stripFirst($LHS.name).c_str() );
      float R = atof( stripFirst($RHS.name).c_str() );
      if( L > R )
      ...
      else if( string($OP.name) == string("<=") )
	{
      ...
      addComment( "UintID relop FloatID: TOC (testing)" );
      addAsm( str_LDY + getNameOf(getAddressOf($LHS.name) ), 3, false );
      addAsm( str_LDA + "#$00", 2, false );
      addAsm( str_JSR + "$B391" + commentmarker + "WORD -> FAC", 3, false );
      cmpFACMEM( "#<" + getNameOf(getAddressOf($RHS.name) ), "#>" + getNameOf(getAddressOf($RHS.name) ));
    ...
    {
      addComment( "UintID relop IntIMM: TOC" );
      addComment( "UintID relop IntIMM: IntIMM is known to be < UintID... OPTIMIZE" );
      if( arg_show_opt )
	{
	  addCompilerMessage( "UintID relop IntIMM: This condition is known at compile-time... just hard-code it!", 0 );
	}
      addAsm( str_LDA + "#$01", 2, false );
      addAsm( str_SEC, 1, false );
  ...
  else if( isUintID($LHS.name) && isUintID($RHS.name))
    {
      addComment( "UintID relop UintID: TOC" );
      addAsm( str_LDA + getNameOf(getAddressOf($LHS.name)), 3, false );
      addAsm( str_CMP + getNameOf(getAddressOf($RHS.name)), 3, false );
    }
  else if( isUintID($LHS.name) && isUintIMM($RHS.name))
    {
      int tmp_addr = getAddressOf($LHS.name);
      string tmp_name = getNameOf(tmp_addr);
      addComment( "UintID relop UintIMM: TOC" );
      // Done
      addAsm( str_LDA + tmp_name, 3, false );
      addAsm( str_CMP + "#$" + toHex( atoi(stripFirst($RHS.name).c_str() )), 2, false );
    }
    ...
    {
      addComment( "UintID relop WordIMM: TOC" );
      
      int V = atoi(stripFirst($RHS.name).c_str() );
      int VL = get_word_L(atoi(stripFirst($RHS.name).c_str()));
      //int VH = get_word_H(atoi(stripFirst($RHS.name).c_str()));
      if( V > 255 )
	{
	  // if > 255 then there's no need for a check
	  if( arg_show_opt )
	    {
	      addCompilerMessage( "UintID relop WordIMM > 255: This condition is known at compile-time... just hard-code it!", 0 );
	...
	{
	  addAsm( str_LDA + getNameOf(getAddressOf($LHS.name)), 3, false );
	  addAsm( str_CMP + "#$" + toHex(VL), 2, false );
	}
    }
      ...
      else if( string($OP.name) == string("<=") )
	{
      ...
      addComment( "UintIMM relop A: TOC" );

      addAsm( str_CMP + "#$" + toHex( atoi(stripFirst($LHS.name).c_str() )), 2, false );
    }
  else if( isUintIMM($LHS.name) && isFAC($RHS.name) )
    {
      addComment( "UintIMM relop FAC: TOC" );
      ...
      else if( string($OP.name) == string("<=") )
	{
      ...
      addComment( "UintIMM relop FloatID: TOC (testing)" );
      inlineFloat($LHS.name);
      cmpFACMEM( "#<" + getNameOf(getAddressOf($RHS.name)), "#>" + getNameOf(getAddressOf($RHS.name)) );
    ...
    {
      addComment( "UintIMM relop FloatIMM: TOC" );
      if( arg_show_opt ) addCompilerMessage( "UintIMM relop FloatIMM: This condition is known at compile-time... just hard-code it!", 0 );
      float L = atof( stripFirst($LHS.name).c_str() );
      float R = atof( stripFirst($RHS.name).c_str() );
      if( L > R )
	{
  ...
  else if( isUintIMM($LHS.name) && isIntID($RHS.name) )
    {
      ...
      if( i > 127 )
	{
	  if( arg_show_opt ) addCompilerMessage( "This condition is known at compile-time... (THIS UintIMM is always > IntID): OPTIMIZE", 0 );
	  addAsm( str_LDA + "#$01", 2, false );
	  addAsm( str_SEC, 1, false );
	}
      else
    ...
    {
      addComment( "UintIMM relop IntIMM: TOC" );
      if( arg_show_opt ) addCompilerMessage( "This condition is known at compile-time... just hard-code it!", 0 );
      float L = atof( stripFirst($LHS.name).c_str() );
      float R = atof( stripFirst($RHS.name).c_str() );
      if( L > R )
    ...
    {
      addComment( "UintIMM relop UintID: TOC");
      addAsm( str_LDA + "#$" + toHex( atoi(stripFirst($LHS.name).c_str() )), 2, false );
      addAsm( str_CMP + getNameOf(getAddressOf($RHS.name)), 3, false );
    }
  else if( isUintIMM($LHS.name) && isUintIMM($RHS.name))
    {
      addComment( "UintIMM relop UintIMM: TOC");
      if( arg_show_opt ) addCompilerMessage( "This condition is known at compile-time... just hard-code it!", 0 );
      float L = atof( stripFirst($LHS.name).c_str() );
      float R = atof( stripFirst($RHS.name).c_str() );
      if( L > R )
    ...
    {
      addComment( "UintIMM relop WordID: TOC" );
      int L = atoi(stripFirst($LHS.name).c_str() );
      addAsm( str_LDA + "#$00", 2, false );
      addAsm( str_CMP + getNameOf(getAddressOf($RHS.name)) + " +1", 3, false );
      addAsm( str_BNE + "!+", 2, false );
      addAsm( str_LDA + "#$" + toHex(L), 2, false );
      addAsm( str_CMP + getNameOf(getAddressOf($RHS.name)), 3, false );
      addAsm( "!:", 0, true );
    ...
    {
      addComment( "UintIMM relop WordIMM: TOC" );
      if( arg_show_opt ) addCompilerMessage( "This condition is known at compile-time... just hard-code it!", 0 );
      float L = atof( stripFirst($LHS.name).c_str() );
      float R = atof( stripFirst($RHS.name).c_str() );
      if( L > R )
    ...
    {
      addComment( "UintIMM relop XA: TOC (self-modifying code)" );
      addAsm( str_STA + "!+", 3, false );
      int L = atoi(stripFirst($LHS.name).c_str() );
    ...
    {
      addComment( "WordID relop A: TOC (self-modifying code)" );
      addAsm( str_STA + "!+", 3, false );
      addAsm( str_SEC );
  ...
  else if( isWordID($LHS.name) && isFAC($RHS.name) )
    {
      ...
      if( string($OP.name) == string("<") )
	{
	  addCompilerMessage( "WordID relop FAC: Relative Operator Manipulation", 0 );
	  strcpy( $OP.name, ">" );
	}
      else if( string($OP.name) == string(">") )
	{
    ...
    {
      addComment( "WordID relop IntID: TOC" );
      addAsm( str_LDA + getNameOf(getAddressOf($LHS.name)) + " +1", 3, false );
      addAsm( str_BNE + "!+", 2, false ); 
      addAsm( str_LDA + getNameOf(getAddressOf($RHS.name)), 3, false );
      addAsm( str_BMI + "!+", 2, false ); 
  ...
  else if( isWordID($LHS.name) && isIntIMM($RHS.name) )
    {
      if( arg_show_opt )
	{
	  addCompilerMessage( "WordID relop IntIMM: IntIMM is ALWAYS < WordID", 0 );
	}
    ...
    {
      int R = atoi( stripFirst($RHS.name).c_str() );
      addComment( "WordID relop UintIMM: TOC" );
      addAsm( str_LDA + getNameOf(getAddressOf($LHS.name)) + " +1",3, false ); 
      addAsm( str_BNE + "!+", 2, false );
      addAsm( str_LDA + getNameOf(getAddressOf($LHS.name)), 3, false );
      addAsm( str_CMP + "#$" + toHex(R), 2, false );
    ...
    {
      addComment( "WordID relop WordID: TOC" );
      addAsm( str_LDA + getNameOf(getAddressOf($LHS.name)) + " +1", 3, false );
      addAsm( str_CMP + getNameOf(getAddressOf($RHS.name)) + " +1", 3, false );
      addAsm( str_BNE + "!+", 2, false );
      addAsm( str_LDA + getNameOf(getAddressOf($LHS.name)), 3, false );
    ...
    {
      addComment( "WordIMM relop FloatIMM: TOC" );
      if( arg_show_opt ) addCompilerMessage( "This condition is known at compile-time... just hard-code it!", 0 );
      float L = atof( stripFirst($LHS.name).c_str() );
      float R = atof( stripFirst($RHS.name).c_str() );
      if( L > R )
    ...
    {
      addComment( "WordIMM relop IntID: TOC" );
      int i = atoi(stripFirst($LHS.name).c_str());
      if( i > 127 )
	{
	  if( arg_show_opt ) addCompilerMessage( "This condition is known at compile-time... (THIS WordIMM is always > IntID): OPTIMIZE", 0 );
	  addAsm( str_LDA + "#$01", 2, false );
	  addAsm( str_SEC, 1, false );
	}
      else
    ...
    {
      addComment( "WordIMM relop IntIMM: TOC" );
      if( arg_show_opt ) addCompilerMessage( "This condition is known at compile-time... just hard-code it!", 0 );
      float L = atof( stripFirst($LHS.name).c_str() );
      float R = atof( stripFirst($RHS.name).c_str() );
      if( L > R )
    ...
    {
      addComment( "WordIMM relop UintID: TOC" );
      int i = atoi(stripFirst($LHS.name).c_str());
      if( i > 255 )
	{
	  if( arg_show_opt ) addCompilerMessage( "This condition is known at compile-time... (THIS WordIMM is always > UintID): OPTIMIZE", 0 );
	  addAsm( str_LDA + "#$01", 2, false );
	  addAsm( str_SEC, 1, false );
	}
      else
    ...
    {
      addComment( "WordIMM relop UintIMM: TOC" );
      if( arg_show_opt ) addCompilerMessage( "This condition is known at compile-time... just hard-code it!", 0 );
      float L = atof( stripFirst($LHS.name).c_str() );
      float R = atof( stripFirst($RHS.name).c_str() );
      if( L > R )
    ...
    {
      addComment( "WordIMM relop WordID: TOC" );
      int i = atoi(stripFirst($LHS.name).c_str());
      int i_L = get_word_L(i);
      int i_H = get_word_H(i);
      addAsm( str_LDA + "#$" + toHex(i_H), 2, false );
      addAsm( str_CMP + getNameOf(getAddressOf($RHS.name)) + " +1", 3, false );
      addAsm( str_BNE + "!+", 2, false );
      addAsm( str_LDA + "#$" + toHex(i_L), 2, false );
      addAsm( str_CMP + getNameOf(getAddressOf($RHS.name)), 3, false );
      addAsm( "!:", 0, true );
    }
  else if( isWordIMM($LHS.name) && isWordIMM($RHS.name) )
    {
      addComment( "WordIMM relop WordIMM: TOC" );
      if( arg_show_opt ) addCompilerMessage( "This condition is known at compile-time... just hard-code it!", 0 );
      float L = atof( stripFirst($LHS.name).c_str() );
      float R = atof( stripFirst($RHS.name).c_str() );
      if( L > R )
...
statement: datatype ID init
{
  ...
  if( _init == string("Slipstream"))
  ...
  else if( isUintDT(_dt) && isUintID(_id) && isXA(_init) )
    {
      addComment( "uint UintID = XA" );
      addCompilerMessage("Initialising a 1 byte memory location with a 2 byte value; losing High Byte", 1);
      int instr_size = 3;
      if( current_variable_base_address < 256 ) instr_size = 2;
      addAsm( str_STA + _id, instr_size, false );
    }
  else if( isUintDT(_dt) && isUintID(_id) && isIntIMM(_init) )
    {
      addComment( "uint UintID = IntIMM" );
  ...
  else if( isIntDT(_dt) && isIntID(_id) && isIntIMM(_init) )
    {
    ...
    {
      addComment( "word WordID = IntIMM" );
      addCompilerMessage("Initialising Unsigned Word with Signed Intger... chaos may ensue!", 1);
  ...
  else if( isIntDT(_dt) && isIntID(_id) && isXA(_init) )
    {
      addComment( "int IntID = XA" );
      addCompilerMessage("Initialising a 1 byte memory location with a 2 byte value; losing High Byte", 1);
      int instr_size = 3;
      if( current_variable_base_address < 256 ) instr_size = 2;
      addAsm( str_STA + _id, instr_size, false );
    ...
    {
      addCompilerMessage( "initialising IntID with FAC ... possible sign conflict... you will likely be eaten by a Grue.", 1 );
...
| ID '[' expression {if(isA($3.name)){addAsm(str_PHA);}} ']' init
{
  ...
  if( ( isUintID(arg0.c_str()) || isIntID(arg0.c_str()) ) && (isUintID(ag1) || isIntID(arg1.c_str())) && isA(arg2.c_str()) )
    {
      addComment( "(U)IntID_array[(U)IntID] = A" ); 
      int addr_of_index = getAddressOf(arg1.c_str());
      addAsm( str_LDX + getNameOf( getAddressOf(arg1.c_str()) ), 3, false );
      addAsm( str_STA + getNameOf( getAddressOf(arg0.c_str()) ) + ",X", 3, false );
    }
  ...
  if( isXA($3.name) )
    {
      ...
      addAsm( "!:\t" + str_BYTE + "$BD" + commentmarker + "<-- LDA abs,X", 1, true );
      addAsm( "!:\t" + str_BYTE + "$00", 1, true );
      addAsm( "!:\t" + str_BYTE + "$00", 1, true );
  ...
  else if( isWordID( $3.name ) )
    {
      ...
      addAsm( "!:\t" + str_BYTE + "$BD" + commentmarker + "<-- LDA abs,X", 1, true ); // LDA abs,X
      addAsm( "!:\t" + str_BYTE + "$00", 1, true );
      addAsm( "!:\t" + str_BYTE + "$00", 1, true );
...
init: '=' expression
{
  ...
  if( isXA($2.name)  )
    ...
    {
      addComment( "The special case of IntIMM" );
      strcpy($$.name, $2.name );

      
      // this is a special case and should maybe be taken out
      // until: dt id init
      //addComment( "initialising with IntIMM" );
      //int v = twos_complement( atoi( stripFirst($2.name).c_str() )) ;
      //if( v < 0 )
      //{
...

{
  ...
  if( isA($1.name) && isA($4.name) )
	...
	{
	  addCompilerMessage( "A - FAC --> FAC (Destroys ZP $02)", 1 );
	  addComment( "A - FAC --> FAC (Destroys $02)" );
	  addAsm( str_STA + "$02", 2, false );
	  pushFAC();
	  addAsm( str_LDY + "$02", 2, false );	  
	  addAsm( str_LDA + "#$00", 2, false );
      ...
      else if( op == string( "/" ) )
	{
	  ...
	  switch( tmp_v )
	    {
	    case 128:
	      addComment( "A / 128 can be done more efficiently!" );
	...
	{
	  // TODO: Can probably be made more efficient (like A - A -> A)
	...
	{
	  addCompilerMessage("FloatID arith IntID: Unknown Operation", 3);
	}
      strcpy( $$.name, "_FAC" );
	...
	{
	  addCompilerMessage( "FloatID math XA: Unknown Operation", 3);
	}
    }
	...
	{
	  addCompilerMessage( "FloatIMM math A: Unknown Operation", 3);
	}
    }
	...
	{
	  addCompilerMessage("FloatIMM math FloatID: Unknown Operation", 3);
	}
      strcpy($$.name, "_FAC" );
    }
    ...
    {
      // TODO: Test and rework for efficiency
	...
	{
	  addComment( "IntID - A --> A (12 cycles)" );
	  addAsm( str_STA + "!+", 3, false ); // 4
      ...
      else
	{
      ...
      if( op == string("+") )
	{
	  addCompilerMessage( "IntID + UintID is an 8-bit operation", 1);
      ...
      else if( op == string("-") )
	{
	  ...
	  addAsm( str_SBC + O2, 3, false );	  
	  addAsm( str_TAY, 1, false );
	  addAsm( str_TXA, 1, false );
	  addAsm( str_SBC + O2 + " +1", sizeOP2A, false );
	  addAsm( str_TAX, 1, false );
	  addAsm( str_TYA, 1, false );	  
	  strcpy($$.name, "_XA");
	}
	...
	{
	  addComment( "IntID - WordIMM --> XA" );
	  addCompilerMessage( "IntID - WordIMM: gives a SIGNED WORD... you will likely be eaten by a grue.", 1 );
	  int tmp = atoi( stripFirst($4.name).c_str() );
	  int tmpL = get_word_L( tmp );
	  int tmpH = get_word_H( tmp );
	  addAsm( str_LDX + "#$00", 2, false );
	  addAsm( str_LDA + O1, sizeOP1A, false );
	  addAsm( str_BPL + "!+", 2, false );
	  addAsm( str_DEX, 1, false );
	  addAsm( "!:", 0, true );	  
	  addAsm( str_SEC );
	  addAsm( str_SBC + "#$" + toHex(tmpL), 2, false );
	  addAsm( str_TAY, 1, false );
	  addAsm( str_TXA, 1, false );	  
	  addAsm( str_SBC + "#$" + toHex(tmpH), 2, false );
	  addAsm( str_TAX, 1, false );
	  addAsm( str_TYA, 1, false );	  
	  strcpy($$.name, "_XA");
	}
      ...
      else if( op == string("*") )
	{
	  ...
	  addAsm( str_LDA + "#$00", 2, false );
	  addAsm( str_STA + "_MUL16_FC", 3, false );
	  addAsm( str_LDA + "#$" + toHex(twos_complement(OP1value)), 2, false );
	  addAsm( str_STA + "_MUL16_FD", 3, false );
	  addAsm( str_LDA + "#$FF", 2, false );
	  addAsm( str_STA + "_MUL16_FE", 3, false );
	  addAsm( str_JSR + "MUL16", 3, false );
      ...
      else if( op == string("**") )
	{
	  ...
	  // mul16_is_needed = true;
	  pow16_is_needed = true;
	  addAsm( str_LDA + "#$" + toHex(twos_complement(op1)), 2, false );
	  addAsm( str_PHA, 1, false );
	  addAsm( str_LDA + "#$FF", 2, false );
	  addAsm( str_PHA, 1, false );
	  addAsm( str_LDA + "$02", 2, false );
	...
	{
	  addCompilerMessage( "Raising a negative number to a fractional power may give complex result and is not handled well.  You will likely be eaten by a grue.", 1 );
	  fPwrT();
	}
      else
	{
	...
	{
	  addCompilerMessage("IntIMM math IntID: Unknown Operation", 3 );
	}
    }
	...
	{
	  addCompilerMessage("IntIMM math UintID: Unknown Operation", 3 );
	}
    }
      ...
      else if( op==string("**"))
	{
	  int tmp_int = atoi(stripFirst($1.name).c_str() );
	  addComment( "IntIMM ** WordID --> XA" );
	  addCompilerMessage( "IntIMM ** WordID: exponent restricted to low byte", 1 );
	  int op1 = atoi(stripFirst($1.name).c_str());
	  op1 = -1 * op1;
      ...
      else if( op == string( "-" ) )
	{
	  ...
	  addAsm( str_LDA + "#$" + toHex(OP1value), 2, false ); 
	  addAsm( str_BYTE + "$E9" + commentmarker + "<-- SBC imm", 1, false );
	  addAsm( "!:\t" + str_BYTE + "$00" + commentmarker + "<-- A", 1, true );
	  addAsm( str_TAY, 1, false );
	  addAsm( str_LDA + "#$FF", 2, false );
	  addAsm( str_BYTE + "$E9" + commentmarker + "<-- SBC imm", 1, false );
	  addAsm( "!:\t" + str_BYTE + "$00" + commentmarker + "<-- X", 1, true );	  
	  addAsm( str_TAX, 1, false );
	  addAsm( str_TYA, 1, false );
	  strcpy( $$.name, "_XA" );
	}
	...
	{
	  addComment( "UintID * IntIMM --> XA" );

	  int tmp_int = atoi(stripFirst($1.name).c_str());

	  int addr_op1 = hexToDecimal($1.name);
	  int op2 = atoi(stripFirst($4.name).c_str());
	  string op1 = getNameOf(getAddressOf($1.name));
	  ...
	  switch( op2 )
	    ...
	    case 0:
	      ...
	      break;
	    case -1:
	      addComment( "Special Case: UintID * -1" );
	      addAsm( str_LDA + op1, 3, false );
	      addAsm( str_EOR + "#$FF", 2, false );
	      addAsm( str_CLC, 1, false );
	      addAsm( str_ADC + "#$01", 2, false );	      
	    ...
	    default:
	      ...
	      addAsm( str_STA + "_MUL16_FB", 3, false );
	      addAsm( str_STX + "_MUL16_FC", 3, false );
	      addAsm( str_LDA + "#$" + toHex(twos_complement(op2)), 2, false );
	      addAsm( str_STA + "_MUL16_FD", 3, false );
	      addAsm( str_DEX, 1, false );
	      addAsm( str_STX + "_MUL16_FE", 3, false );
	      addAsm( str_JSR + "MUL16", 3, false );
      ...
      else if( op == string("*"))
	{
	  ...
	  if( tmp_v == 0 )
	    {
	      addAsm( commentmarker + str_LDA + "#$00", 0, false );
	      addAsm( commentmarker + str_TAX, 0, false );	      
	      addAsm( str_LAX + "#$00", 2, false );
	    }
	  else if( tmp_v == 1 )
	    {
	      addComment( "UintID * UintIMM --> XA special case: * 1" );
	      addAsm( str_LDA + O1, sizeOP1A, false);
	      addAsm( str_LDX + "#$00", 2, false );
	    }
	  else if( tmp_v == 2 )
	    ...
	    {
	      addComment( "UintID * UintIMM --> XA special case: * 8" );
	      addAsm( str_LDA + O1, sizeOP1A, false);
	    ...
	    {
	      addComment( "UintID * UintIMM --> XA special case: * 11" );
	      if( !arg_unsafe_math )
		{
		  addComment( "multiply by 11 - 145 cycles" );
		  addComment( "save ZP $02/$03/$04/$05" );
		  addAsm( str_LDA + "$02", 2, false ); // 3
		  addAsm( str_PHA, 1, false ); // 3
		  addAsm( str_LDA + "$03", 2, false );  //3
		  addAsm( str_PHA, 1, false ); //3 
		  addAsm( str_LDA + "$04", 2, false );  //3
		  addAsm( str_PHA, 1, false ); //3
		  addAsm( str_LDA + "$05", 2, false ); //3
		  addAsm( str_PHA, 1, false ); //3
		}
	      else
		{
		  addComment( "multiply by 11 (unsafe in ZP) - 93 cycles" );
		}
	      addComment( "----------------------" );
	      addAsm( str_LDA + O1, sizeOP1A, false );
	      ...
	      if( !arg_unsafe_math )
		{
		  ...
		  addAsm( str_PLA, 1, false );  //4
		  addAsm( str_STA + "$04", 2, false );  //3
		  addAsm( str_PLA, 1, false );  //4
		  addAsm( str_STA + "$03", 2, false );  //3
		  addAsm( str_PLA, 1, false );   // 4
		  addAsm( str_STA + "$02", 2, false );   //3
		}
	      addAsm( str_TYA, 1, false );
	      ...
	      if( !arg_unsafe_math )
		{
	      ...
	      addAsm( str_ADC + O1, sizeOP1A, false );
	      addAsm( str_BCC + "!+", 2, false );
	      addAsm( str_INC + "$03", 2, false );
	      addAsm( "!:\t" + str_ASL, 1, true ); // double
	      addAsm( str_ROL + "$03", 2, false );
	      addAsm( str_ASL, 1, false ); // double
	      addAsm( str_ROL + "$03", 2, false );
	      addAsm( str_ASL, 1, false ); // double
      ...
      else if( op == string("*") )
	{
	  switch( OP2 )
	    {
	    case 0:
	      addComment( "Special Case: UintID * 0x0000 --> XA" );
	      addAsm( str_LAX + "#$00", 1, false );
	      break;
	    case 256:
	      addComment( "Special Case: UintID * 0x0100 --> XA" );
	      addAsm( str_TAX, 1, false );
	      addAsm( str_LDA + "#$00", 1, false );
	      break;
	    case 512:
	      addComment( "Special Case: UintID * 0x0200 --> XA" );
	      addAsm( str_ASL, 1, false );
	      addAsm( str_TAX, 1, false );
	      addAsm( str_LDA + "#$00", 1, false );
	      break;
	    case 1024:
	      addComment( "Special Case: UintID * 0x0400 --> XA" );
	      addAsm( str_ASL, 1, false );
	      addAsm( str_ASL, 1, false );
	      addAsm( str_TAX, 1, false );
	      addAsm( str_LDA + "#$00", 1, false );
	      break;
	    case 2048:
	      addComment( "Special Case: UintID * 0x0800 --> XA" );
	      addAsm( str_ASL, 1, false );
	      addAsm( str_ASL, 1, false );
	      addAsm( str_ASL, 1, false );
	      addAsm( str_TAX, 1, false );
	      ...
	      break;
	    case 4096:
	      addComment( "Special Case: UintID * 0x1000 --> XA" );
	      addAsm( str_ASL, 1, false );
	      addAsm( str_ASL, 1, false );
	      addAsm( str_ASL, 1, false );
	      addAsm( str_ASL, 1, false );
	...
	{
	  addCompilerMessage( "UintID math WordIMM: Unknown Operation", 3 );
	}
    }
      ...
      if( op == string("+"))
	{
	  ...
	  if( tmp_int != 0 )
	    ...
	    {
	      addCompilerMessage( "Adding 0x00 to IntID... no operation", 1 );
	    }
	  strcpy($$.name, "_A" );
	}
      else if( op == string("-"))
	{
	  addCompilerMessage( "UintIMM - IntID: possible unexpected results.", 1 );
	  int tmp_int = atoi(stripFirst($1.name).c_str());
	...
	{
	  addComment( "WordID ** UintID --> XA" );
	  addCompilerMessage( "WordID ** UintID cant't have UintID > 0x0F, the result is > 0xFFFF", 0 );
	  pow16_is_needed = true;
	...
	{
	  addComment( "WordID ** WordID --> XA" );
	  addCompilerMessage( "WordID ** WordID: Exponent restricted to low-byte of WordID (op2)", 1 );
	  pow16_is_needed = true;
      ...
      if( op == string( "+" ) )
	{
	  ...
	  addAsm( str_BPL + "!+", 2, false );
	  addAsm( str_DEX, 1, false );
	  addAsm( "!:\t" + str_ADC + "#$" + toHex(get_word_L(tmp_int)), 2, true );
	  addAsm( str_TAY, 1, false );
	  addAsm( str_TXA, 1, false );
	...
	{
	  addCompilerMessage( "WordIMM ** XA: exponent restricted to only Low-byte", 1 );
	  mul16_is_needed = true;
	  pow16_is_needed = true;
	  addAsm( str_TAY, 1, false );	  
	  addAsm( str_LDA + "#$" + toHex(OP1L), 2, false );
      ...
      else if( op == string("*") )
	{
	  ...
	  switch(tmp_v)
	    ...
	    case 32768:
	      ...
	      break;
	    case 16384:
	      if( !special_case_flag ) addComment( "Special Case: XA * WordIMM (16384) --> XA" );
	      special_case_flag = true;
	      addAsm( str_ASL );                 // 2 cycles
	    case 8192:
	      if( !special_case_flag ) addComment( "Special Case: XA * WordIMM (8192) --> XA" );
	      special_case_flag = true;
	      addAsm( str_ASL );                 // 2 cycles
	    case 4096:
	      if( !special_case_flag ) addComment( "Special Case: XA * WordIMM (4096) --> XA" );
	      special_case_flag = true;
	      addAsm( str_ASL );                 // 2 cycles
	    case 2048:
	      if( !special_case_flag ) addComment( "Special Case: XA * WordIMM (2048) --> XA" );
	      special_case_flag = true;
	      addAsm( str_ASL );                 // 2 cycles
	    case 1024:
	      if( !special_case_flag ) addComment( "Special Case: XA * WordIMM (1024) --> XA" );
	      special_case_flag = true;
	      addAsm( str_ASL );                 // 2 cycles
	    case 512:
	      if( !special_case_flag ) addComment( "Special Case: XA * WordIMM (512) --> XA" );
	      special_case_flag = true;
	      addAsm( str_ASL );                 // 2 cycles
	    case 256:
	      if( !special_case_flag ) addComment( "Special Case: XA * WordIMM (256) --> XA" );
	      special_case_flag = true;
	      addAsm( str_TAX );                 // 2 cycles
	      addAsm( str_LDA + "#$00", 2, false );  // 2 cycles
	      strcpy($$.name, "_XA");
	    ...
	    case 128:
	      ...
	      if( !special_case_flag ) addComment( "Special Case: XA * WordIMM (128) --> XA" );
	      special_case_flag = true;
	    case 64:
	      ...
	      break;
	    case 32:
	      if( !special_case_flag ) addComment( "Special Case: XA * WordIMM (32) --> XA" );
	      special_case_flag = true;
	      addAsm( str_ASL );
	      addAsm( str_TAY );
	      addAsm( str_TXA );
	      ...
	      addAsm( str_TYA );
	    case 16:
	      if( !special_case_flag ) addComment( "Special Case: XA * WordIMM (16) --> XA" );
	      special_case_flag = true;
	      addAsm( str_ASL );
	      addAsm( str_TAY );
	      addAsm( str_TXA );
	      ...
	      addAsm( str_TYA );
	    case 8:
	      if( !special_case_flag ) addComment( "Special Case: XA * WordIMM (8) --> XA" );
	      special_case_flag = true;
	      addAsm( str_ASL );
	      addAsm( str_TAY );
	      addAsm( str_TXA );
	      ...
	      addAsm( str_TYA );
	    case 4:
	      if( !special_case_flag ) addComment( "Special Case: XA * WordIMM (4) --> XA" );
	      special_case_flag = true;
	      addAsm( str_ASL );
	      addAsm( str_TAY );
	      addAsm( str_TXA );
	      ...
	      addAsm( str_TYA );
	    case 2:
	      if( !special_case_flag ) addComment( "Special Case: XA * WordIMM (2) --> XA" );
	      special_case_flag = true;
	      addAsm( str_ASL );
	      addAsm( str_TAY );
	      addAsm( str_TXA );
	      ...
	      break;
	    case 5:
	      addComment( "Special Case: XA * 0x0005 --> XA" );
	      if( !arg_unsafe_math )
		{
		  addAsm( str_TAY );
		  addAsm( str_LDA + "$02", 2, false );
	      ...
	      if( !arg_unsafe_math )
		{
	      ...
	      break;
	    case 3:
	      addComment( "Special Case: XA * 0x0003 --> XA" );
	      ...
	      if( !arg_unsafe_math )
		{
	      ...
	      break;
	    case 10:
	      addComment( "Special Case: XA * 0x000A --> XA" );
	      if( !arg_unsafe_math )
		{
		  addAsm( str_TAY );
		  addAsm( str_LDA + "$02", 2, false );
...
| tROL '(' expression ')'
{
  if( isUintID($3.name) || isIntID($3.name)  )
    ...
    {
      addComment( "ror( WordIMM (address) ) -> A" );
      addAsm( str_ASL + "$" + toHex(atoi(stripFirst($3.name).c_str())), 3, false );
      addAsm( str_BCC + "!+", 2, false );
      addAsm( str_ORA + "#$01", 2, false );
      addAsm( "!:\t" + str_LDA + "$" + toHex(atoi(stripFirst($3.name).c_str())), 3, true );
      strcpy( $$.name, "_A" );
    }
...
| tLSR '(' expression ')'
{
  ...
  if( isA( $3.name ) )
    ...
    {
      addComment( "lsr( WordIMM (address) ) -> A" );
      addAsm( str_LDA + "$" + toHex(atoi(stripFirst($3.name).c_str())), 3, false );
      addAsm( str_LSR, 1, false );
      addAsm( str_LDA + "$" + toHex(atoi(stripFirst($3.name).c_str())), 3, false );
      strcpy( $$.name, "_A" );
  ...
  if( isUintIMM( $5.name ) || isIntIMM( $5.name ))
    {
      ...
      addAsm( str_STA + "!++", 3, false );
      addAsm( str_LDX + "#$" + toHex(atoi(stripFirst($5.name).c_str())), 2, false );
      addAsm( str_BYTE + "$BD"+ commentmarker + "<-- LDA abs,X", 1, false );
      addAsm( "!:\t" + str_BYTE + "$00", 1, true );
      addAsm( "!:\t" + str_BYTE + "$00", 1, true );
    }
...
| tRND '(' expression ')' 
{
  ...
  if( !sidrnd_is_initialised  )
    {
      addComment( "initialize random number generator" );
      addAsm( str_JSR + "SIDRND", 3, false );
      sidrnd_is_initialised = true;
    }
...
| expression tbwOR expression
{
  ...
  else if( isA($1.name) && isIntIMM($3.name) )
    {
      addComment( "A | IntIMM" );
...
| expression tbwAND expression
{
  ...
  if( isWordID($1.name) && isWordID($3.name) )
  ...
  else if( isXA($1.name) && isWordIMM($3.name) )
    {
      ...
      addAsm( str_AND + "#$" + toHex(vL), 2, false);
      addAsm( str_TAY, 1, false );
      addAsm( str_TXA, 1, false );
      addAsm( str_AND + "#$" + toHex(vH), 2, false);
      addAsm( str_TAX, 1, false );
      addAsm( str_TYA, 1, false );
      strcpy( $$.name, "_XA" );
    }  
...
| tTOUINT '(' expression ')'
{
  ...
  if( isFloatID($3.name))
    ...
    {
      addCompilerMessage( "For the love of might... just hard-code this value as a UintIMM!", 3 );
    }
  else if( isIntID($3.name) )
    {
      addComment( "touint(IntID) --> A");
      addCompilerMessage( "touint(IntID) --> A: may produce unexpected results", 1 );
      int base_address =  getAddressOf($3.name);
      int inst_size = 3;
      if( base_address < 256 ) inst_size = 2;
      addAsm( str_LDA + getNameOf(base_address), inst_size, false  );
    }
  else if( isWordID($3.name) || isUintID($3.name) )
    {
      addComment( "touint(WordID|UintID) --> A");
...
| tABS '(' expression ')'
{
  ...
  if( isFAC($3.name) )
    ...
    {
      addCompilerMessage( "For the love of might... just hard-code the absolute value!", 3 );
    }
  else
    {
      addCompilerMessage( "cannot calculate absolute value of unknown type", 3);
    }
}
| tSIN '(' expression ')'
{
...
| tSPRITECOLLISION '(' expression ')'
{
  ...
  if( isUintID($3.name) )
  ...
  else if( isA( $3.name ) )
    {
      addAsm( str_STA + "!+", 3, false );
      addAsm( str_LDA + "$D01E" + commentmarker + "MOB-MOB Collision Register", 3, false );
      addAsm( str_BYTE + "$29" + commentmarker + "<-- AND imm", 1, false );
      addAsm( "!:\t" + str_BYTE + "$00", 1, true );
   }
  else
    {
...
| tGETH '(' expression ')'
{
  ...
  else if( isA( $3.name ) || isUintID( $3.name) || isUintIMM($3.name) )
    {
      addComment( "The \"Hi\" byte of an unsigned integer is always #$00" );
      addCompilerMessage( "The \"Hi\" byte of an unsigned integer is always #$00", 1 );
      addAsm( str_LDA + "#$00", 2, false );
    }
  else if( isIntIMM($3.name) )
    {
      addCompilerMessage( "The \"Hi\" byte of a signed immediate integer is always #$FF", 1 );
      addComment( "The \"Hi\" byte of a signed immediate integer is always #$FF" );
      addAsm( str_LDA + "#$FF", 2, false );
    }
  else if( isIntID($3.name) )
    {
      addComment( "Determine what \"would be\" the hi-byte" );
      addAsm( str_LDX + "#$00", 2, false );
      addAsm( str_LDA + getNameOf(getAddressOf($3.name)), 3, false );
      addAsm( str_TAY, 1, false );
      addAsm( str_ASL, 1, false );
...
| tGETL '(' expression ')'
{
  ...
  if( isWordID( $3.name ) )
    ...
    {
      addCompilerMessage("getting lo-byte of A not necessary", 0 );
    }
  else
    {
...
| tSQRT '(' expression ')'
{
  ...
  if( isFloatID( $3.name ) )
	...
	{
	  addComment( "sqrt(uintid) optimized for size, not speed") ;
	  addAsm( str_LDA + getNameOf(getAddressOf($3.name)), 3, false );      
    ...
    {
      addCompilerMessage( "NYI: sqrt(XA)", 3 );
    }
  else
    {
  ...
  if( isWordID($3.name) )
    {
      ...
      addAsm( str_STA + "!+", 3, false );
      addAsm( str_STX + "!++", 3, false );
      addAsm( str_BYTE + "$AD" + commentmarker + "<-- LDA abs", 1, false );
      addAsm( "!:\t" + str_BYTE + "$00" + commentmarker + "lo byte", 1, true );
      addAsm( "!:\t" + str_BYTE + "$00" + commentmarker + "hi byte", 1, true );
    ...
    {
      addComment( "peek( IntID );" );
      addCompilerMessage( "peek(IntID): using abs( IntID )", 1 );
      addAsm( str_LDA + getNameOf(getAddressOf($3.name)), 3, false );
      addAsm( str_AND + "#$7F", 2, false );
      addAsm( str_STA + "!+", 3, false );
